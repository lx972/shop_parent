...
import cn.lx.shop.order.service.OrderItemService;
...
public class OrderItemController {
    ...
    public PageResult<OrderItem> findPage(@RequestBody(required = false) @ApiParam(name = "OrderItem对象",value = "传入JSON数据",required = false) OrderItem orderItem, @PathVariable  int page, @PathVariable  int size){
        //调用OrderItemService实现分页条件查询OrderItem
        PageInfo<OrderItem> pageInfo = orderItemService.findPage(orderItem, page, size);
        return new PageResult<OrderItem>(pageInfo.getTotal(),pageInfo.getList());
    }
    ...
    public PageResult<OrderItem> findPage(@PathVariable  int page, @PathVariable  int size){
