...
import cn.lx.shop.order.pojo.ReturnCause;
import cn.lx.shop.order.service.ReturnCauseService;
...
public class ReturnCauseController {
    ...
    public PageResult<ReturnCause> findPage(@RequestBody(required = false) @ApiParam(name = "ReturnCause对象",value = "传入JSON数据",required = false) ReturnCause returnCause, @PathVariable  int page, @PathVariable  int size){
        //调用ReturnCauseService实现分页条件查询ReturnCause
        PageInfo<ReturnCause> pageInfo = returnCauseService.findPage(returnCause, page, size);
        return new PageResult<ReturnCause>(pageInfo.getTotal(),pageInfo.getList());
    }
    ...
    public PageResult<ReturnCause> findPage(@PathVariable  int page, @PathVariable  int size){
