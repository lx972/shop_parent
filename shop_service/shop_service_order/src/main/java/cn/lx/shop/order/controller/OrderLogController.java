...
import cn.lx.shop.order.pojo.OrderLog;
import cn.lx.shop.order.service.OrderLogService;
...
public class OrderLogController {
    ...
    public PageResult<OrderLog> findPage(@RequestBody(required = false) @ApiParam(name = "OrderLog对象",value = "传入JSON数据",required = false) OrderLog orderLog, @PathVariable  int page, @PathVariable  int size){
        //调用OrderLogService实现分页条件查询OrderLog
        PageInfo<OrderLog> pageInfo = orderLogService.findPage(orderLog, page, size);
        return new PageResult<OrderLog>(pageInfo.getTotal(),pageInfo.getList());
    }
    ...
    public PageResult<OrderLog> findPage(@PathVariable  int page, @PathVariable  int size){
