...
import cn.lx.shop.order.pojo.ReturnOrder;
import cn.lx.shop.order.service.ReturnOrderService;
...
public class ReturnOrderController {
    ...
    public PageResult<ReturnOrder> findPage(@RequestBody(required = false) @ApiParam(name = "ReturnOrder对象",value = "传入JSON数据",required = false) ReturnOrder returnOrder, @PathVariable  int page, @PathVariable  int size){
        //调用ReturnOrderService实现分页条件查询ReturnOrder
        PageInfo<ReturnOrder> pageInfo = returnOrderService.findPage(returnOrder, page, size);
        return new PageResult<ReturnOrder>(pageInfo.getTotal(),pageInfo.getList());
    }
    ...
    public PageResult<ReturnOrder> findPage(@PathVariable  int page, @PathVariable  int size){
