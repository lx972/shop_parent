...
import cn.lx.shop.order.pojo.UndoLog;
import cn.lx.shop.order.service.UndoLogService;
...
public class UndoLogController {
    ...
    public PageResult<UndoLog> findPage(@RequestBody(required = false) @ApiParam(name = "UndoLog对象",value = "传入JSON数据",required = false) UndoLog undoLog, @PathVariable  int page, @PathVariable  int size){
        //调用UndoLogService实现分页条件查询UndoLog
        PageInfo<UndoLog> pageInfo = undoLogService.findPage(undoLog, page, size);
        return new PageResult<UndoLog>(pageInfo.getTotal(),pageInfo.getList());
    }
    ...
    public PageResult<UndoLog> findPage(@PathVariable  int page, @PathVariable  int size){
