...
import cn.lx.shop.order.pojo.OrderConfig;
import cn.lx.shop.order.service.OrderConfigService;
...
public class OrderConfigController {
    ...
    public PageResult<OrderConfig> findPage(@RequestBody(required = false) @ApiParam(name = "OrderConfig对象",value = "传入JSON数据",required = false) OrderConfig orderConfig, @PathVariable  int page, @PathVariable  int size){
        //调用OrderConfigService实现分页条件查询OrderConfig
        PageInfo<OrderConfig> pageInfo = orderConfigService.findPage(orderConfig, page, size);
        return new PageResult<OrderConfig>(pageInfo.getTotal(),pageInfo.getList());
    }
    ...
    public PageResult<OrderConfig> findPage(@PathVariable  int page, @PathVariable  int size){
