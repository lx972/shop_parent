...
import cn.lx.shop.order.pojo.TaskHis;
import cn.lx.shop.order.service.TaskHisService;
...
public class TaskHisController {
    ...
    public PageResult<TaskHis> findPage(@RequestBody(required = false) @ApiParam(name = "TaskHis对象",value = "传入JSON数据",required = false) TaskHis taskHis, @PathVariable  int page, @PathVariable  int size){
        //调用TaskHisService实现分页条件查询TaskHis
        PageInfo<TaskHis> pageInfo = taskHisService.findPage(taskHis, page, size);
        return new PageResult<TaskHis>(pageInfo.getTotal(),pageInfo.getList());
    }
    ...
    public PageResult<TaskHis> findPage(@PathVariable  int page, @PathVariable  int size){
