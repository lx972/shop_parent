...
import cn.lx.shop.user.pojo.Provinces;
import cn.lx.shop.user.service.ProvincesService;
...
public class ProvincesController {
    ...
    public PageResult<Provinces> findPage(@RequestBody(required = false) @ApiParam(name = "Provinces对象",value = "传入JSON数据",required = false) Provinces provinces, @PathVariable  int page, @PathVariable  int size){
        //调用ProvincesService实现分页条件查询Provinces
        PageInfo<Provinces> pageInfo = provincesService.findPage(provinces, page, size);
        return new PageResult<Provinces>(pageInfo.getTotal(),pageInfo.getList());
    }
    ...
    public PageResult<Provinces> findPage(@PathVariable  int page, @PathVariable  int size){
