...
public class AddressController {
    ...
    public PageResult<Address> findPage(@RequestBody(required = false) @ApiParam(name = "Address对象",value = "传入JSON数据",required = false) Address address, @PathVariable  int page, @PathVariable  int size){
        //调用AddressService实现分页条件查询Address
        PageInfo<Address> pageInfo = addressService.findPage(address, page, size);
        return new PageResult<Address>(pageInfo.getTotal(),pageInfo.getList());
    }
    ...
    public PageResult<Address> findPage(@PathVariable  int page, @PathVariable  int size){
