...
import cn.lx.shop.user.pojo.Cities;
import cn.lx.shop.user.service.CitiesService;
...
public class CitiesController {
    ...
    public PageResult<Cities> findPage(@RequestBody(required = false) @ApiParam(name = "Cities对象",value = "传入JSON数据",required = false) Cities cities, @PathVariable  int page, @PathVariable  int size){
        //调用CitiesService实现分页条件查询Cities
        PageInfo<Cities> pageInfo = citiesService.findPage(cities, page, size);
        return new PageResult<Cities>(pageInfo.getTotal(),pageInfo.getList());
    }
    ...
    public PageResult<Cities> findPage(@PathVariable  int page, @PathVariable  int size){
