...
import cn.lx.shop.user.pojo.Areas;
import cn.lx.shop.user.service.AreasService;
...
public class AreasController {
    ...
    public PageResult<Areas> findPage(@RequestBody(required = false) @ApiParam(name = "Areas对象",value = "传入JSON数据",required = false) Areas areas, @PathVariable  int page, @PathVariable  int size){
        //调用AreasService实现分页条件查询Areas
        PageInfo<Areas> pageInfo = areasService.findPage(areas, page, size);
        return new PageResult<Areas>(pageInfo.getTotal(),pageInfo.getList());
    }
    ...
    public PageResult<Areas> findPage(@PathVariable  int page, @PathVariable  int size){
