package cn.lx.shop.content.controller;
import cn.lx.shop.content.pojo.Content;
import cn.lx.shop.content.service.ContentService;
...
public class ContentController {
    ...
    public PageResult<Content> findPage(@RequestBody(required = false) @ApiParam(name = "Content对象",value = "传入JSON数据",required = false) Content content, @PathVariable  int page, @PathVariable  int size){
    ...
    public PageResult<Content> findPage(@PathVariable  int page, @PathVariable  int size){
    ...
    public Result<List<Content>> findByCategoryId(@PathVariable("id") Long id){
        ...
        //调用ContentService实现查询Content
        List<Content> list = contentService.findList(content);
        return new Result<List<Content>>(true, StatusCode.OK,"根据分类id查询广告集合",list) ;
    }
}
