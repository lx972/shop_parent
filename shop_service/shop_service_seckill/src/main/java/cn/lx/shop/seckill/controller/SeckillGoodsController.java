package cn.lx.shop.seckill.controller;
...
public class SeckillGoodsController {
    ...
    public Result queryStatus(){
        ...
        SeckillStatus seckillStatus = seckillGoodsService.queryStatus(username);
     ...
     * @return
     */
    @PostMapping(value = "/user/add/order")
    public Result userOrder(@RequestParam String seckillStartTime,@RequestParam String skuId){
        String username = TokenDecode.decodeToken().get("username");
        seckillGoodsService.userOrder(seckillStartTime,skuId,username);
     ...
     * @return
     */
    @GetMapping(value = "/redis/{seckillStartTime}/{skuId}")
    public Result getRedisGoodById(@PathVariable("seckillStartTime")String seckillStartTime,@PathVariable("skuId") String skuId){
        SeckillGoods seckillGood=seckillGoodsService.getRedisGoodById(seckillStartTime,skuId);
        return new Result(true,StatusCode.OK,"查询秒杀商品的详情成功", seckillGood);
    }
     ...
     * @return
     */
    @GetMapping(value = "/list/{seckillStartTime}")
    public Result list(@PathVariable("seckillStartTime") String seckillStartTime){
        List<SeckillGoods> seckillGoodsList=seckillGoodsService.list(seckillStartTime);
        return new Result(true,StatusCode.OK,"获取对应秒杀时间的商品列表成功", seckillGoodsList);
    }
     ...
     * @return
     */
    @GetMapping(value = "/date/menu")
    public Result getDateMenu(){
        return new Result(true,StatusCode.OK,"获取秒杀时间菜单成功", DateUtil.getDateMenus());
    }
    ...
    public PageResult<SeckillGoods> findPage(@RequestBody(required = false) @ApiParam(name = "SeckillGoods对象",value = "传入JSON数据",required = false) SeckillGoods seckillGoods, @PathVariable  int page, @PathVariable  int size){
        ...
        return new PageResult<SeckillGoods>(pageInfo.getTotal(),pageInfo.getList());
    }
    ...
    public PageResult<SeckillGoods> findPage(@PathVariable  int page, @PathVariable  int size){
